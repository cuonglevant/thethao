...
export default function TeamPage() {
  ...
  const displayName = team?.name || slug.replace(/^\d+-/, "").replace(/-/g, " ");
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-blue-600">
              {team?.area?.name || "Chưa có thông tin"} • Thành lập: {team?.founded || "N/A"}
            </p>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p><span className="font-medium">Tên đầy đủ:</span> {team.name}</p>
                <p><span className="font-medium">Tên ngắn:</span> {team.shortName}</p>
                <p><span className="font-medium">Thành lập:</span> {team.founded}</p>
                <p><span className="font-medium">Sân vận động:</span> {team.venue}</p>
              </div>
              <div>
                <h3 className="font-medium mb-2">Liên hệ</h3>
                <p><span className="font-medium">Địa chỉ:</span> {team.address}</p>
                <p><span className="font-medium">Website:</span> {team.website}</p>
                <p><span className="font-medium">Màu sắc:</span> {team.clubColors}</p>
              </div>
            </div>
          ) : (
            <p>Không có thông tin chi tiết về đội bóng.</p>
          )}
        </div>
      </div>
    </div>
